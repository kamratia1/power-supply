...
/*void process_command(void)
{
  static char cmd[16] = {0};    // assume command is 16 bytes long
  static int cmd_ptr = 0;       // command pointer
  char c = 0;                   // incoming character
  
  int cmd_number = 0;           // command number
  int val = 0;                  // value to set for command
  
  // Process Characters
  if (HAL_UART_Receive_DMA(&UartHandle, (uint8_t *)RxBuffer, 1) == HAL_OK)
  {
    c = *RxBuffer;
    //UART_print((char*)RxBuffer);      // echo's incoming character for debugging
    cmd[cmd_ptr++] = c;
    
    // If a command is entered (determined by the last character "+")
    if (c == '+'){
      
      sscanf(cmd, "cmd,%d,%d,+", &cmd_number, &val);
      switch(cmd_number)
      {
        case SET_VSW:
          break;
